...
class FishFarmEnv:
    ...
    def step(self, actions):
        ...
        for i in range(len(self.state_data["oxygen"])):
            ...
            self.state_data["oxygen"][i] += temperature_effect(
                self.state_data["temperature"]
            )

            self.state_data["oxygen"][i] = max(
                0, min(10, self.state_data["oxygen"][i])
            )
